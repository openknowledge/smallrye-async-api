...
public class AsyncApiDocumentService {
  ...
  void create() throws IOException {
    ...
    ClassLoader cl = AsyncApiConstants.classLoader == null ? Thread.currentThread().getContextClassLoader() : AsyncApiConstants.classLoader;
    ...
    try (InputStream is = cl.getResourceAsStream(AsyncApiConstants.BASE_NAME + AsyncApiFormat.JSON)) {
      if (is != null) {
        try (AsyncApiStaticFile staticFile = new AsyncApiStaticFile(is, AsyncApiFormat.JSON)) {
          ...
          this.jsonDocument = AsyncApiSerializer.serialize(document.get(), AsyncApiFormat.JSON).getBytes(StandardCharsets.UTF_8);
          this.yamlDocument = AsyncApiSerializer.serialize(document.get(), AsyncApiFormat.YAML).getBytes(StandardCharsets.UTF_8);
          document.reset();
          document = null;
        }
      } else {
