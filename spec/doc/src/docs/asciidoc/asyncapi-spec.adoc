...
https://github.com/openknowledge/smallrye-async-api/blob/master/core/src/main/java/io/smallrye/asyncapi/core/api/AsyncApiConfig.java[AASConfig] class.
...
Vendors are required to fetch a single document named `asyncapi` with an extension
...
only place a single `asyncapi` document into that folder.
